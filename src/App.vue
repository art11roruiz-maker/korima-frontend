...
// --- CONFIGURATION ---
const backendUrl = "https://korima-backend-service-305564772278.us-east1.run.app";

// --- STATE MANAGEMENT ---
const briefing = ref(null);
const conversationHistory = ref([]);
const isListening = ref(false);
const activeTab = ref('briefing'); // 'briefing', 'memory', or 'about'
...
function startListening() {
  ...
  recognition.onresult = async (event) => {
    ...
    await sendTranscriptToBrain(transcript);
  };
...
async function sendTranscriptToBrain(text) {
  try {
    ...
    activeTab.value = 'memory'; // Switch to memory tab after interaction
  } catch (error) {
...
<template>
  <div class="app-container">
    <header class="app-header">
      <img src="https://i.imgur.com/gJeEwjg.png" alt="Kórima Logo" class="logo">
      <h1>Kórima</h1>
    </header>

    <main class="app-main">
      <div class="mic-container">
        <button @click="startListening" :class="{ 'listening': isListening }" class="mic-button" title="Presiona para hablar">
          🎤
        </button>
      ...
      <nav class="tabs">
        <button :class="{ active: activeTab === 'briefing' }" @click="activeTab = 'briefing'">Briefing</button>
        <button :class="{ active: activeTab === 'memory' }" @click="activeTab = 'memory'">Memoria</button>
        <button :class="{ active: activeTab === 'about' }" @click="activeTab = 'about'">Acerca De</button>
      </nav>
      ...
      <div class="tab-content">
        <div v-if="activeTab === 'briefing'">
          <section v-if="briefing" class="card">
            ...
            <div v-else>
              <p>{{ briefing.focus.description }}</p>
              <ul v-if="briefing.focus.events">
                <li v-for="event in briefing.focus.events" :key="event">{{ event }}</li>
              </ul>
            </div>
          </section>
          <div v-else class="card">
            <p>Cargando briefing desde el cerebro de Kórima...</p>
          </div>
        </div>
            ...
            <div v-if="conversationHistory.length > 0">
              <ul>
                <li v-for="(entry, index) in conversationHistory" :key="index">
                  <p><strong>Tú:</strong> {{ entry.user }}</p>
                  <p><strong>Kórima:</strong> {{ entry.korima }}</p>
                </li>
              </ul>
            </div>
        ...
        <div v-if="activeTab === 'about'">
           <section class="card">
            ...
            <p>Esta es una aplicación de IA multimodal y proactiva, construida desde cero por ti.</p>
             <p><strong>Frontend:</strong> Desplegado en Vercel.</p>
             <p><strong>Backend:</strong> Desplegado en Google Cloud Run.</p>
             <p><strong>Memoria:</strong> Almacenada en Google Firestore.</p>
          </section>
        </div>
